...
export default function (state = weather_today, action) {
    switch (action.type) {
        case "weather_today_update": weather_today_update()
            break
        case "get_weather_today": get_weather_today()
            break
    }
...
async function weather_today_update() {
    await axios
...
async function lerArquivo(caminho) {
    ...
    await rnfs.readDir(caminho)
        ...
        .then(async (contents) => {
            weather_today = JSON.parse(contents)
        })
        .catch((err) => {
